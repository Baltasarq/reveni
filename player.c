...
/** Ends the game */
void end_game(Player * player, const char * msg)
{
...
void show_hud(Player * player)
{
    int o2_ink = INK_GREEN;
    int energy_ink = INK_GREEN;
    int timer_ink = INK_GREEN;
...
void proc2(Player * player)
{
    ...
    if ( player->o2 < 1 ) {
        end_game( player, "Al principio, te parece que no pasa nada, "
                          ...
                          "que te marees y caigas al suelo. Solo "
                          "un rato... un ratito..." );
    }
...
void resp(Player * player, Order * order)
{
    ...
    Obj * obj_agujas = locate_obj_by_id( "agujas" );
      ...
      && order->obj1 == obj_suit )
    {
        ...
        end_game( player,
                  ...
                  "aire abandona tu traje, y poco a poco, te cuesta "
                  "respirar y te sientes ahogado.\nAntes de perder "
                  "el conocimiento, te das cuenta de que has muerto." );
    }
    else
...
void do_loc_desc(Player * player)
{
    const int num_loc = player->num_loc;
